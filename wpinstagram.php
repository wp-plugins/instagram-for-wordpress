...
class WPInstagram_Widget extends WP_Widget {
	function WPInstagram_Widget(){
		...
		if(is_active_widget('','','wpinstagram-widget')&&!is_admin()):
			...
			wp_enqueue_style("wpinstagram", $this->wpinstagram_path."wpinstagram.css", Array(), '0.2.5');
			if(in_array('fancybox-for-wordpress/fancybox.php',(array)get_option('active_plugins',array()))==false):
				...
				wp_enqueue_script("wpinstagram", $this->wpinstagram_path."js/wpinstagram.js", Array('jquery', 'jquery.cycle', 'fancybox'), '0.3');
			else:
				wp_enqueue_script("wpinstagram", $this->wpinstagram_path."js/wpinstagram-without-fancybox.js", Array('jquery',  'jquery.cycle'), '0.3');
			endif;
		endif;
	...
	function instagram_login($login, $pass){
		...
		curl_setopt ($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt ($ch, CURLOPT_POSTFIELDS, "username=" . $login . "&password=" . $pass . "&grant_type=password&client_id=90c2afb9762041138b620eb56710ca39&client_secret=c605ec6443e348e68643470fdc3ef02a");
	...
	function instagram_get_latest($instance){
		...
		if($instance['access_token'] != null):
			$ch = curl_init("https://api.instagram.com/v1/users/self/media/recent?access_token=".$instance['access_token']);
			if(ini_get("open_basedir") == ""):
				curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
			endif;
			curl_setopt($ch, CURLOPT_HEADER, 0);
